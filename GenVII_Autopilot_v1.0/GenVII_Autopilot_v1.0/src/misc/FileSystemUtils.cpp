/*******************************************************************************
 *
 *   Copyright 2007-2014 NetBurner, Inc.  ALL RIGHTS RESERVED
 *
...
char EffsErrorCode[][80] = { "F_NO_ERROR", // 0
...
int OpenMMCFlash(int drv) {
	...
	if (get_cd(drv) == 0) {
		while (get_cd(drv) == 0) {
			if (drv == MMC_OFF_BOARD)
				iprintf(
						"No off-board MMC/SD card detected. Insert a card and then press ");
			else
				iprintf(
						"No on-board MMC/SD card detected. Insert a card and then press ");
	...
	if (get_wp(drv) == 1) {
		while (get_wp(drv) == 1) {
			iprintf(
					"SD/MMC Card is write-protected. Disable write protection, then ");
			iprintf("press <Enter>\r\n");
			if (getchar() == 13)
				break;
		}
 ...
 * FormatExtFlash
 */
BYTE FormatExtFlash(int drv, long FATtype) {
	int rv;
